...
echo ====================================================
echo     Toto's Audiophile Toolkit Automated Installer
echo ====================================================
echo.
...
net session >nul 2>&1
if %errorlevel% neq 0 (
    echo [ERROR] This script requires administrative permissions to set up system paths.
...
if %errorlevel% eq 0 (
    echo [FOUND] FFmpeg is already installed and configured on your system!
    goto RUN_TOOLKIT
)

echo [1/4] FFmpeg not detected. Preparing automated installation...
if not exist "C:\ffmpeg" mkdir "C:\ffmpeg"

:: 3. Download the pre-compiled binary package
echo [2/4] Downloading latest stable FFmpeg package...
curl -L -o "%TEMP%\ffmpeg.zip" "https://gyan.dev"
if %errorlevel% neq 0 (
...
:: 4. Extract archive using built-in Windows utility
echo [3/4] Extracting components to C:\ffmpeg...
powershell -Command "Expand-Archive -Path '%TEMP%\ffmpeg.zip' -DestinationPath 'C:\ffmpeg' -Force"

:: Clean up the long extracted folder hierarchy to maintain standard C:\ffmpeg\bin
for /d %%D in ("C:\ffmpeg\ffmpeg-*") do (
    xcopy /E /Y "%%D\*" "C:\ffmpeg\" >nul
    rd /S /Q "%%D"
)
del /Q "%TEMP%\ffmpeg.zip"

:: 5. Inject Path into System Variables permanently
echo [4/4] Configuring Windows System Environment Variables...
setx /M PATH "%PATH%;C:\ffmpeg\bin"
set "PATH=%PATH%;C:\ffmpeg\bin"

echo.
echo ====================================================
echo SUCCESS: FFmpeg installation and setup complete!
echo ====================================================
echo.
pause
...
:RUN_TOOLKIT
:: ====================================================
:: LAUNCH TOTO'S AUDIOPHILE TOOLKIT V4.0 CODE DIRECTLY
:: ====================================================
:MENU
cls
echo ====================================================
...
echo ====================================================
echo.
echo Select the directory containing your 24-bit FLAC files...
...
echo.
echo Select the target output folder for your WAV files...
